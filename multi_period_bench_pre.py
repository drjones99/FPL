# Builds on Sertalp Cay's pre-season FPL optimisation problem by adding bench weights
# Source: https://github.com/sertalpbilal/FPL-Optimization-Tools/blob/main/src-pre-season/multi_period.py
...
def solve_multi_period_pre_season_fpl(horizon, objective='regular', decay_base=0.84, no_transfer=False):
    ...
    players_no_gk = merged_data[merged_data.element_type != 1].index.to_list()
    ...
    bench = model.add_variables(players, gameweeks, priority, name='bench', vartype=so.binary)
    ...
    bench_weights = [0.03, 0.21, 0.02, 0.002]
    ...
    model.add_constraints((lineup[p,w] + so.expr_sum(bench[p,w,l] for l in priority) == squad[p,w] for p in players for w in gameweeks), name='bench_1')
    ...
    gw_total = {w: gw_xp[w] + gw_bench_xp[w] - 4 * penalized_transfers[w] for w in gameweeks}
